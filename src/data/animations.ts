...
type Seed = {
  id: string;
  name: string;
  category: keyof typeof descriptionMap;
  scene: string;
};
...
export type AnimationItem = Seed & {
  ...
  componentLoader?: () => Promise<MotionModule>;
};
...
const motionModules = import.meta.glob<MotionModule>("../motions/*/index.vue");
...
export const animations: AnimationItem[] = seeds.map((seed, index) => {
...
export const animationCategories: AnimationCategory[] = Array.from(
...
export const findAnimationById = (id: string): AnimationItem | undefined =>
  animations.find((item) => item.id === id);
